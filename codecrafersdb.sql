...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkGameImage` (IN `imageIN` VARCHAR(100), OUT `countOUT` INT(1))   SELECT COUNT(game.id)
INTO countOUT
FROM game
WHERE game.image = imageIN$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `checkGameName` (IN `nameIN` VARCHAR(100), OUT `countOUT` INT(1))   SELECT COUNT(game.id)
INTO countOUT
FROM game
WHERE game.name = nameIN$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteGame` (IN `idIN` INT(9))   DELETE
FROM game
WHERE game.id = idIN$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `login` (IN `userNameIN` VARCHAR(100), IN `passwordIN` TEXT, OUT `userIdOUT` INT(9), OUT `userNameOUT` VARCHAR(100), OUT `lastNameOUT` VARCHAR(100), OUT `firstNameOUT` VARCHAR(100), OUT `emailOUT` VARCHAR(100), OUT `adminOUT` INT(1))   SELECT 
	user.id,
    user.userName,
    user.lastName,
    user.firstName,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `newGame` (IN `nameIN` VARCHAR(100), IN `priceIN` INT(9), IN `descriptionIN` TEXT, IN `imageIN` VARCHAR(100), IN `ageLimitIN` INT(2), IN `discountIN` INT(3), IN `inStockIN` INT(5), IN `deviceIdIN` INT(9), IN `platformIdIN` INT(9))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateGame` (IN `idIN` INT(9), IN `nameIN` VARCHAR(100), IN `priceIN` INT(9), IN `descriptionIN` TEXT, IN `imageIN` VARCHAR(100), IN `ageLimitIN` INT(2), IN `discountIN` INT(3), IN `inStockIN` INT(5), IN `deviceIdIN` INT(9), IN `platformIdIN` INT(9))   BEGIN
    ...
    UPDATE game
    SET game.name = nameIN,
        game.price = priceIN,
        game.description = descriptionIN,
        game.image = imageIN,
        game.ageLimit = ageLimitIN,
        game.discount = discountIN,
        game.inStock = inStockIN,
        game.deviceId = deviceIdIN,
        game.platformId = platformIdIN,
        game.updatedAt = CURRENT_TIMESTAMP
    WHERE game.id = idIN;
...
INSERT INTO `user` (`id`, `userName`, `lastName`, `firstName`, `email`, `password`, `admin`, `createdAt`, `updatedAt`) VALUES
(1, 'SunnyDay21', 'Kéri', 'Bence', 'keribence0@gmail.com', '0282b275f47e80afc7aa184b3e0f39c091a0b4eb8c8a1ff03bce806fc8e7c4df', 1, '2024-02-18 17:23:22', NULL),
(2, 'AdventureSeeker', 'Nagy', 'Péter', 'peter.nagy@example.com', '78f35a60ccd8402035f62f8790ee4f6a3ace2ea357240196ccf4e74e5c04e4e0', 0, '2024-02-18 17:24:11', NULL),
(3, 'MusicLover88', 'Tóth', 'Eszter', 'eszter.toth@example.com', '5e35a60e4432b66f04ebdfee06ab303c6459ab2884b0438838d7588b4c740ba1', 0, '2024-02-18 17:24:46', NULL);
